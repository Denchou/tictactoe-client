...
var gameStats = function gameStats() {
  return $.ajax({
    url: config.apiUrl + '/games',
    method: 'GET',
    headers: {
...
var replay = function replay(event) {
  return $.ajax({
...
var gameHandler = __webpack_require__(7);
var userHandler = __webpack_require__(12);
...
var gameEvent = __webpack_require__(8);
...
var gameUi = __webpack_require__(9);
var store = __webpack_require__(1);
...
var onNewGame = function onNewGame(event) {
  ...
  gameApi.newGame().then(gameUi.onNewGameSuccess).catch(gameUi.onNewGameFailure);
};
...
var onPlay = function onPlay(event) {
  ...
  if (!store.board.game.over && event.target.innerHTML !== '') {
    ...
    gameApi.play(event).then(gameUi.onPlaySuccess(event)).catch(gameUi.onPlayFailure);
  } else {
...
var onGameStats = function onGameStats(event) {
  event.preventDefault();
  gameApi.gameStats().then(gameUi.onGameStatsSuccess).catch(gameUi.onGameStatsFailure);
};
var onReplay = function onReplay(event) {
  event.preventDefault();
  gameApi.replay(event).then(gameUi.onReplaySuccess).catch(gameUi.onReplayFailure);
};
module.exports = {
  onNewGame: onNewGame,
  onPlay: onPlay,
  onGameStats: onGameStats,
  onReplay: onReplay
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var store = __webpack_require__(1);
var gameLogic = __webpack_require__(10);
var statsFunctions = __webpack_require__(11);
var config = __webpack_require__(2);
...
var onPlaySuccess = function onPlaySuccess(event) {
  if (store.tag === 'X') {
...
var onPlayFailure = function onPlayFailure() {
  $('#message').html('Something went wrong with the game, please check your internet connection.');
};

var onGameStatsSuccess = function onGameStatsSuccess(stats) {
  store.games = stats.games;
  var complete = store.games.filter(function (over) {
    return over.over;
  });
  var incomplete = store.games.filter(function (over) {
    return !over.over;
  });
  incomplete.sort(function (a, b) {
    return a.id - b.id;
  });
  var Xwins = complete.filter(function (win) {
    return statsFunctions.giveWinner(win.cells) === 'X';
  });
  var Owins = complete.filter(function (win) {
    return statsFunctions.giveWinner(win.cells) === 'O';
  });
...
var onGameStatsFailure = function onGameStatsFailure() {
  $('#message').html('Something went wrong with the game, please refresh your browser.');
};
var onReplaySuccess = function onReplaySuccess(response) {
  $('.box').empty().removeClass('x o diagwin colwin rowwin tie');
...
var onReplayFailure = function onReplayFailure(response) {
...
module.exports = {
  ...
  onGameStatsSuccess: onGameStatsSuccess,
  onGameStatsFailure: onGameStatsFailure,
...
var checkRow = function checkRow() {
  var board = store.board.game.cells;
...
var checkCol = function checkCol() {
  var board = store.board.game.cells;
...
var checkDia = function checkDia() {
  var board = store.board.game.cells;
...
var checkOver = function checkOver() {
  var board = store.board.game.cells;
  var isOver = store.board.game.over;
  if (isOver) {
    ...
    var audio = new Audio('https://github.com/Denchou/tictactoe-client/blob/master/Draw.m4a?raw=true');
    audio.play();
    $('.box').addClass('tie');
    return true;
  }
...
var gameBoard = function gameBoard(event) {
  var board = store.board.game.cells;
  var winState = store.board.game.over;
  if (winState) {
...
var giveWinner = function giveWinner(board) {
  if (row(board)) {
    return row(board);
  } else if (col(board)) {
    return col(board);
...
var countX = function countX(board) {
  var count = board.filter(function (x) {
    return x === 'X';
  });
  return count.length;
};

var countO = function countO(board) {
  var count = board.filter(function (o) {
    return o === 'O';
  });
  return count.length;
};

var populate = function populate(board) {
  for (var i = 0; i < board.length; i++) {
    $('#' + i).html(board[i]);
  }
};
module.exports = {
  giveWinner: giveWinner,
  countX: countX,
  countO: countO,
  populate: populate
};
...
var userEvent = __webpack_require__(13);
...
var getFormFields = __webpack_require__(14);
